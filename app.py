...
with tab4:
    ...
    if 'data_validos' in locals() and data_validos and any(t in ['FTIR-Acetato', 'FTIR-Cloroformo'] for t in tipos_sel):
        ...
        for idx, (muestra, tipo, x, y) in enumerate(data_validos):
            ...
            if datos["peso"] > 0:
                indice_oh_acetato = (y_3548 - datos["acetato"]) * 52.5253 / datos["peso"] if datos["acetato"] != 0 else "—"
                indice_oh_cloroformo = (y_3611 - datos["cloroformo"]) * 66.7324 / datos["peso"] if datos["cloroformo"] != 0 else "—"
            else:
