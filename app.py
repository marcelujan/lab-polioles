...
with tab2:
    ...
    x_stats = df_sel[df_sel["Tipo"] == tipo_x].groupby("Nombre")["Valor"].agg(["mean", "std"])
    y_stats = df_sel[df_sel["Tipo"] == tipo_y].groupby("Nombre")["Valor"].agg(["mean", "std"])
    comunes = x_stats.index.intersection(y_stats.index)
