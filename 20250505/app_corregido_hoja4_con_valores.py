...
with tab4:
    ...
    if 'data_validos' in locals() and data_validos and any(t in ['FTIR-Acetato', 'FTIR-Cloroformo'] for t in tipos_sel):
        ...
        for muestra, tipo, x, y in data_validos:
            ...
            # Buscar datos auxiliares (senal_3548, senal_3611, peso_muestra) del espectro
            espectro_muestra = next((e for e in muestras if e["nombre"] == muestra), None)
            if espectro_muestra:
                espectro_tipo = next((esp for esp in espectro_muestra.get("espectros", []) if esp.get("tipo") == tipo), None)
            else:
                espectro_tipo = None
            ...
            if tipo == "FTIR-Acetato" and y_3548 is not None and peso_muestra and senal_3548 is not None:
                indice_oh_acetato = (y_3548 - senal_3548) * 52.5253 / peso_muestra
                indice_oh_cloroformo = "—"
            elif tipo == "FTIR-Cloroformo" and y_3611 is not None and peso_muestra and senal_3611 is not None:
                indice_oh_acetato = "—"
                indice_oh_cloroformo = (y_3611 - senal_3611) * 66.7324 / peso_muestra
            else:
                indice_oh_acetato = indice_oh_cloroformo = "—"
