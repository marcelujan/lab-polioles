...
with tab4:
    ...
    if not df_imagenes.empty:
        st.subheader("Espectros de imágenes")
        for _, row in df_imagenes.iterrows():
            try:
                binario = base64.b64decode(row["Contenido"])
                st.image(BytesIO(binario), caption=f"{row['Muestra']} – {row['Tipo']}", use_container_width=True)
            except Exception as e:
                st.error(f"No se pudo cargar imagen de {row['Muestra']}: {e}")
