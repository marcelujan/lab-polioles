...
#    st.title("Análisis FTIR")
    ...
    # 3.b Gráfico cruzado Modelo vs FTIR + T
    if st.checkbox("Modelo vs Índice OH (nuevo gráfico)", value=True):
...
def render_grafico_cruzado_modelo_oh():
    ...
    st.markdown("### Índice OH (modelo) vs FTIR + T")

    if "oh_bridge_state" not in st.session_state:
        st.session_state["oh_bridge_state"] = {
            "a": 1.0, "b": 0.0, "mostrar_T": True, "mostrar_OL": True,
            "curvas_seleccionadas": sorted(df_oh["Curva"].dropna().unique().tolist())
        }
    S = st.session_state["oh_bridge_state"]

    colA, colB, colC, colD = st.columns([1,1,1,1])
    with colA:
        S["a"] = st.number_input("a (escala modelo→Índice)", value=float(S.get("a",1.0)), step=0.1, format="%.3f", key="oh_a")
    with colB:
        S["b"] = st.number_input("b (offset)", value=float(S.get("b",0.0)), step=0.1, format="%.3f", key="oh_b")
    with colC:
        S["mostrar_OL"] = st.checkbox("Mostrar Índice (modelo)", value=bool(S.get("mostrar_OL", True)), key="oh_m_ol")
    with colD:
        S["mostrar_T"] = st.checkbox("Mostrar T(°C)", value=bool(S.get("mostrar_T", True)), key="oh_m_t")

    curvas = sorted([c for c in df_oh["Curva"].dropna().unique().tolist() if c != ""] or ["Sin curva"])
    S["curvas_seleccionadas"] = st.multiselect("Curvas FTIR a mostrar", curvas, default=S.get("curvas_seleccionadas", curvas))

    df_vis = df_oh.copy()
    df_vis = df_vis[df_vis["Curva"].isin(S["curvas_seleccionadas"])] if S["curvas_seleccionadas"] else df_vis
    if "Usar" not in df_vis.columns:
        df_vis["Usar"] = True
    df_edit = st.data_editor(
        df_vis[["Curva","X","Índice OH","Observaciones","Usar"]].sort_values(["Curva","X"]),
        hide_index=True,
        use_container_width=True,
        num_rows="dynamic",
        key="oh_bridge_editor"
    )
    try:
        keys = ["Curva","X","Índice OH","Observaciones"]
        merged = df_oh.merge(df_edit[keys+["Usar"]], on=keys, how="left", suffixes=("",""))
        df_oh["Usar"] = merged["Usar"].fillna(df_oh.get("Usar", True))
        st.session_state["df_oh_editado"] = df_oh
    except Exception:
        pass
    ...
    if S["mostrar_T"] and T_C.size == times_h.size:
        fig.add_trace(go.Scatter(x=times_h, y=T_C, mode="lines", name="T (°C)", yaxis="y2"))

    df_plot = df_oh[df_oh.get("Usar", True)] if "Usar" in df_oh.columns else df_oh
    for curva, grupo in df_plot.groupby("Curva" if "Curva" in df_plot else ""):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=grupo["Curva"].astype(str)
        ))

    fig.update_layout(
        title="Modelo 2-films vs Índice OH (FTIR)",
