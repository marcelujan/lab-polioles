                    ...
                    if df is not None:
                        ...
                        for idx_zona, zona in enumerate(zonas):
                            ...
                            try:
                                ...
                                # Buscar espectro 1D correspondiente
                                # Primero intentar con el mismo nombre pero sin "D"
                                nombre_1d = nombre_archivo.replace("_RMN 1H D_", "_RMN 1H_")
                                espectros_1d = df_espectros_muestra[
                                    (df_espectros_muestra["archivo"] == nombre_1d) & 
                                    (df_espectros_muestra["tipo"] == "RMN 1H")
                                ]
                                
                                # Si no encuentra, buscar cualquier RMN 1H de la misma muestra
                                if espectros_1d.empty:
                                    espectros_1d = df_espectros_muestra[
                                        (df_espectros_muestra["tipo"] == "RMN 1H")
                                    ]
                                
                                if not espectros_1d.empty:
                                    row_espectro_1d = espectros_1d.iloc[0]
                                    
                                    # Validar que el contenido no sea None
                                    if row_espectro_1d.get("contenido") is None:
                                        st.warning(f"No se pudo obtener contenido del espectro 1D para {nombre_archivo}")
                                        x_1d = None
                                        y_1d = None
                                    else:
                                    ...
                                    st.warning(f"No se encontró espectro 1D correspondiente para {nombre_archivo}")
                                    x_1d = None
                                    y_1d = None
                            except Exception as e:
