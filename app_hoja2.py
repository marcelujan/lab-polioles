# --- APP COMPLETA: Hoja 1 + Hoja 2 (mejorada) ---
...
# --- HOJA 2: Análisis de datos (mejorada) ---
with tab2:
    ...
    df_seleccion = df[df["ID"].isin(seleccion)]
    st.subheader("Resumen de selección")
    st.dataframe(df_seleccion.drop(columns=["ID"]), use_container_width=True)

    st.subheader("Gráfico XY")
    columnas_validas = ["Nombre", "Tipo", "Valor", "Fecha", "Observaciones"]
    if not df_seleccion.empty and "Valor" in df_seleccion.columns:

        opciones_columnas = [col for col in df_seleccion.columns if col not in ["ID", "Observaciones"]]

        colx, coly = st.columns(2)
        with colx:
            eje_x = st.selectbox("Selección de eje X", opciones_columnas)
        with coly:
            eje_y = st.selectbox("Selección de eje Y", opciones_columnas)
        ...
        if usar_manual_x:
            ...
            for idx, row in df_seleccion.iterrows():
                ...
                nombre = row["Nombre"]
                tipo = row["Tipo"]
                valores_x_manual[clave] = st.number_input(
                    f"{nombre} ({tipo})", key=f"manual_x_{clave}", step=0.1
                )

            x = list(valores_x_manual.values())
            y = df_seleccion[eje_y].tolist()
            nombres = df_seleccion["Nombre"].tolist()
        else:
            try:
                x = df_seleccion[eje_x].tolist()
                y = df_seleccion[eje_y].tolist()
                nombres = df_seleccion["Nombre"].tolist()
            except:
                st.warning("No se pueden convertir los valores seleccionados.")
                x, y = None, None
