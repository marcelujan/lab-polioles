...
with tab5:
    ...
    # --- Reutilizar la selección sincronizada ---
    ...
    if muestras_filtradas:
        for muestra in muestras_filtradas:
            ...
            for espectro in muestra["espectros"]:
                st.write(f"- Tipo: {espectro.get('tipo', '')}")
                st.write(f"  Señal 3548: {espectro.get('senal_3548', 'No disponible')}")
                st.write(f"  Señal 3611: {espectro.get('senal_3611', 'No disponible')}")
                st.write(f"  Peso muestra: {espectro.get('peso_muestra', 'No disponible')}")
                st.divider()
    else:
        st.info("No hay espectros disponibles para calcular Índice OH.")
# --- HOJA 6 ---
with tab6:
    st.title("Consola")
