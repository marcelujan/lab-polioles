...
COSTOS_COLS = ["Agente", "Costo (por kg)", "Costo relativo"]
...
def render_tab11(db, cargar_muestras, guardar_muestra, mostrar_sector_flotante):
    ...
    # calcular relativo (NaHCO3=1)
    precio_dict = {}
    for _, r in df_costos.iterrows():
        agente = str(r["Agente"]).strip()
        precio = _parse_float(r["Costo (por kg)"])
        if agente and precio is not None:
            precio_dict[agente] = precio
    ref = precio_dict.get("NaHCO3")
    if ref and ref > 0:
        df_costos["Costo relativo"] = [
            f"{(_parse_float(r['Costo (por kg)'])/ref):.2f}" if _parse_float(r["Costo (por kg)"]) else ""
            for _, r in df_costos.iterrows()
        ]
    else:
        df_costos["Costo relativo"] = ""
    ...
    df_in.insert(idx, "$", 0)
    df_in["$"] = [ _calcular_coste_fila(r, precio_dict) for _, r in df_in.iterrows() ]
    ...
    colcfg["VOL ACUO (mL)"] = st.column_config.TextColumn(label="Vaq (mL)", width="small")
    ...
    df_edit = st.data_editor(
        ...
        key="down_editor_native", disabled=["$"],
    )

    st.subheader("Costos por kg de agentes")
    df_costos_edit = st.data_editor(
        df_costos, num_rows="dynamic", use_container_width=True,
        hide_index=True,
        column_config={
            "Agente": st.column_config.TextColumn(label="Agente", width="medium"),
            "Costo (por kg)": st.column_config.NumberColumn(label="Costo (por kg)", step=1, format="%.0f"),
            "Costo relativo": st.column_config.TextColumn(label="Costo relativo", disabled=True, width="small"),
        },
        key="down_costos_editor",
        disabled=["Costo relativo"],
    )
