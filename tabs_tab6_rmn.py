...
def render_rmn_plot(df, tipo="RMN 1H", key_sufijo="rmn1h", db=None):
    ...
    x_max = colx2.number_input("X máximo", value=9.0 if tipo == "RMN 1H" else 220.0, key=f"x_max_{key_sufijo}")
    y_min = coly1.number_input("Y mínimo", value=0.0, key=f"y_min_{key_sufijo}")
    y_max = coly2.number_input("Y máximo", value=60.0 if tipo == "RMN 1H" else 2.0, key=f"y_max_{key_sufijo}")
...
# --- Tabla Bibliográfica de señales pico δ (RMN 1H) ---
    ...
    activar_sombra_dt2_1h = st.checkbox("Sombrear Tabla de Cálculos D/T2 (FAMAF) RMN 1H", value=False,key=f"sombra_dt2_1h_{key_sufijo}_{tipo}")
    activar_sombra_dt2_13c = st.checkbox("Sombrear Tabla de Cálculos D/T2 (FAMAF) RMN 13C", value=False, key=f"sombra_dt2_13c_{key_sufijo}_{tipo}")
